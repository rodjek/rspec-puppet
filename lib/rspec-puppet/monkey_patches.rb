...
module Kernel
  ...
  def require(path)
    return if (['puppet/util/windows', 'win32/registry', 'win32/resolv', 'win32ole'].include?(path)) && RSpec::Puppet.rspec_puppet_example? && Puppet::Util::Platform.pretend_windows?
    old_require(path)
  end
end
